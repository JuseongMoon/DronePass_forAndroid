...
- Current local HEAD before this code/documentation update: `9dcd102 Align drone dropdown dismiss behavior`.
- Working tree before this save: flight-zone floating button pulse parity code, regression test, and documentation updates pending.
...
- Latest completed local parity checkpoint: flight-zone floating button count-change pulse parity audit, committed in the current HEAD containing this file.
- Code-work note: iOS `FlightZoneLayerSelector` briefly scales the map-layer floating button to 1.1x when `visibleLayerCount` changes. Android now applies the same count-change pulse to the flight-zone FAB while keeping the existing size, icon, badge, and bottom safe-area placement contracts. `MapFloatingButtonsTest` and `MapScreenLayersTest` passed for this checkpoint.
...
- Current local HEAD before this documentation save: `9dcd102 Align drone dropdown dismiss behavior`.
- Working tree status before this documentation save: flight-zone floating button pulse parity code, regression test, and documentation updates pending.
- Play internal test version already published: `3.5.5 (102) internal-1`.
- Do not rebuild or re-upload version code `102` just to resume the release process.
