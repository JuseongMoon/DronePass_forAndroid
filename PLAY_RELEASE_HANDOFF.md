...
> Latest code/parity checkpoint at this handoff: current HEAD containing the address-search result-card token parity checkpoint
...
- Current local code/parity checkpoint at this documentation save: current HEAD containing the address-search result-card token parity checkpoint.
- Working tree before this save: address-search result-card token parity changes pending.
...
- Latest completed local parity checkpoint: address-search result-card token parity audit, committed in the current HEAD containing this file.
- Code-work note: iOS `AddressCardView` uses 16pt card radius, 8pt shadow radius at 6% black, 10pt vertical row spacing, Jibun blue badge, Road green badge, 6pt badge radius, 8/4 padding, and 14pt building icon. Android `SearchAddressSheet` now exposes and tests those shared tokens so result-card visuals remain pinned to iOS. `SearchAddressSheetTest` passed for this checkpoint.
...
Saved again on 2026-07-05 KST from this repository so a later session can resume the app release process when the user says "앱 출시 과정 다시 이어나가자". The Play Console internal-test release flow had already reached the published version detail page, and local Android/iOS parity audits later continued through the address-search result-card token contract audit. The latest local code/parity checkpoint before this documentation save has not been uploaded as a newer Play build.

- Current working branch: `fix/critical-pri0-fixes`.
- Current local code/parity checkpoint before this documentation save: current HEAD containing the address-search result-card token parity checkpoint.
- Working tree status before this documentation save: address-search result-card token parity changes pending; after the save commit, rerun `git status --short` and expect a clean tree unless the user has made newer edits.
- Play internal test version already published: `3.5.5 (102) internal-1`.
- Do not rebuild or re-upload version code `102` just to resume the release process.
