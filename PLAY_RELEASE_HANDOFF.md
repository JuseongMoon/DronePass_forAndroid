...
> Latest code/parity checkpoint at this handoff: current HEAD containing the address-search clear behavior parity checkpoint
...
- Current local code/parity checkpoint at this documentation save: current HEAD containing the address-search clear behavior parity checkpoint.
- Working tree before this save: address-search clear behavior parity changes pending.
...
- Latest completed local parity checkpoint: address-search clear behavior parity audit, committed in the current HEAD containing this file.
- Code-work note: iOS `SearchAddressView` clear action only clears the search text and does not explicitly reset `addresses` or `errorMessage`, so Android `SearchAddressSheet` now keeps existing results/error state when the clear button is tapped. `SearchAddressSheetTest` passed for this checkpoint.
...
Saved again on 2026-07-05 KST from this repository so a later session can resume the app release process when the user says "앱 출시 과정 다시 이어나가자". The Play Console internal-test release flow had already reached the published version detail page, and local Android/iOS parity audits later continued through the address-search clear behavior contract audit. The latest local code/parity checkpoint before this documentation save has not been uploaded as a newer Play build.

- Current working branch: `fix/critical-pri0-fixes`.
- Current local code/parity checkpoint before this documentation save: current HEAD containing the address-search clear behavior parity checkpoint.
- Working tree status before this documentation save: address-search clear behavior parity changes pending; after the save commit, rerun `git status --short` and expect a clean tree unless the user has made newer edits.
- Play internal test version already published: `3.5.5 (102) internal-1`.
- Do not rebuild or re-upload version code `102` just to resume the release process.
