...
> Latest code/parity checkpoint at this handoff: current HEAD containing the general toast typography parity checkpoint
...
- Current local code/parity checkpoint at this documentation save: current HEAD containing the general toast typography parity checkpoint.
- Working tree before this save: general toast typography parity changes pending.
...
- Latest completed local parity checkpoint: general toast typography parity audit, committed in the current HEAD containing this file.
- Code-work note: iOS `ToastMessageModifier` uses `.font(.subheadline)` for the general bottom toast, so Android `IosToastMessageOverlay` now pins the shared toast text size to 15sp while keeping the existing 2s duration, 300ms move/fade animation, 0.7 black background, 12dp corner radius, 16dp horizontal padding, 10dp vertical padding, and 100dp bottom offset. `WeatherForecastParityTest` passed for this checkpoint.
