...
> Latest code/parity checkpoint at this handoff: current HEAD containing the coordinate/address guide typography parity checkpoint
...
- Current local code/parity checkpoint at this documentation save: current HEAD containing the coordinate/address guide typography parity checkpoint.
- Working tree before this save: coordinate/address guide typography parity changes pending.
...
- Latest completed local parity checkpoint: coordinate/address guide typography parity audit, committed in the current HEAD containing this file.
- Code-work note: iOS coordinate and address guide cards use `subheadline` for the main guide text, `caption` for guide titles, and `caption2` for coordinate examples. Android now pins coordinate guide typography to 15/12/11sp and address guide typography to 15/12/12sp while preserving existing card background, radius, width, padding, and spacing. `SearchAddressSheetTest` and `ShapeEditDefaultsTest` passed for this checkpoint.
...
Saved again on 2026-07-05 KST from this repository so a later session can resume the app release process when the user says "앱 출시 과정 다시 이어나가자". The Play Console internal-test release flow had already reached the published version detail page, and local Android/iOS parity audits later continued through the coordinate/address guide typography contract audit. The latest local code/parity checkpoint before this documentation save has not been uploaded as a newer Play build.

- Current working branch: `fix/critical-pri0-fixes`.
- Current local code/parity checkpoint before this documentation save: current HEAD containing the coordinate/address guide typography parity checkpoint.
- Working tree status before this documentation save: coordinate/address guide typography parity changes pending; after the save commit, rerun `git status --short` and expect a clean tree unless the user has made newer edits.
- Play internal test version already published: `3.5.5 (102) internal-1`.
- Do not rebuild or re-upload version code `102` just to resume the release process.
