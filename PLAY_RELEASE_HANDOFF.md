...
- Current local HEAD before this code/documentation update: `266040d Align flight zone FAB haptic with iOS`.
- Working tree before this save: flight-zone layer selector haptic contract test and documentation updates pending.
...
- Latest completed local parity checkpoint: flight-zone layer selector haptic contract audit, committed in the current HEAD containing this file.
- Code-work note: iOS `FlightZoneLayerSelector` uses medium haptics for bulk select/deselect and light haptics for individual layer toggles. Android now names and tests the same haptic mapping for the layer sheet while preserving the already-aligned flight-zone FAB open haptic. `FlightZoneLayerSelectorTest` and `MapFloatingButtonsTest` passed for this checkpoint.
...
- Current local HEAD before this documentation save: `266040d Align flight zone FAB haptic with iOS`.
- Working tree status before this documentation save: flight-zone layer selector haptic contract test and documentation updates pending.
- Play internal test version already published: `3.5.5 (102) internal-1`.
- Do not rebuild or re-upload version code `102` just to resume the release process.
