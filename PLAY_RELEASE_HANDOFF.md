...
- Current local code checkpoint before this documentation save: `1cc5898 Align VWorld altitude row visibility with iOS`.
- Working tree before this save: release-resume documentation updates pending.
...
- Latest completed local parity checkpoint: VWorld zone detail altitude-row visibility audit, committed in the current HEAD containing this file.
- Code-work note: iOS `VWorldZoneDetailView` shows the altitude row when altitude information exists, even if only upper or lower altitude is present. Android now shows the VWorld altitude row when either formatted upper or lower altitude exists, so single-bound altitude data is no longer hidden. `VWorldZoneDetailSheetTest` and `MapScreenLayersTest` passed for this checkpoint.
