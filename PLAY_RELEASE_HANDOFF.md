...
- Current local HEAD before this code/documentation update: `7454ee4 Use monochrome notification small icon`.
- Working tree at save time: main floating saved-tab icon parity code, regression test, and documentation updates pending.
...
- Latest completed local parity checkpoint: main floating saved-tab icon parity audit, committed in the current HEAD containing this file.
- Code-work note: iOS `MainTabView` uses SF Symbols `map/map.fill`, `tray.full/tray.full.fill`, and `gearshape/gearshape.fill`. Android already matched the tab sizing, scale, labels, and map/settings icon intent; the saved tab now uses `Icons.Outlined.Inbox`/`Icons.Filled.Inbox` instead of `Inventory2`, which is the closest Material counterpart to iOS `tray.full`. `MainScreenStartDestinationTest` passed for this checkpoint.
