...
- Current local HEAD before this code/documentation update: `1ee151b Align flight zone FAB pulse with iOS`.
- Working tree before this save: flight-zone floating button open haptic parity code, regression test, and documentation updates pending.
...
- Latest completed local parity checkpoint: flight-zone floating button open haptic parity audit, committed in the current HEAD containing this file.
- Code-work note: iOS `FlightZoneLayerSelector` sends a light haptic when opening the layer sheet. Android now performs the matching light haptic on flight-zone FAB click while keeping the existing size, icon, badge, pulse, and bottom safe-area placement contracts. `MapFloatingButtonsTest`, `FlightZoneLayerSelectorTest`, and `MapScreenLayersTest` passed for this checkpoint.
...
- Current local HEAD before this documentation save: `1ee151b Align flight zone FAB pulse with iOS`.
- Working tree status before this documentation save: flight-zone floating button open haptic parity code, regression test, and documentation updates pending.
- Play internal test version already published: `3.5.5 (102) internal-1`.
- Do not rebuild or re-upload version code `102` just to resume the release process.
