...
- Latest completed local parity checkpoint: sketch ViewModel mode/delete ordering audit, committed in the current HEAD containing this file.
- Code-work note: iOS `SketchManager` resets the drawing buffer and undo/redo stacks on sketch-mode entry, saves an in-progress line before mode exit sync, and stores delete-all as one undo action while clearing redo. Android already matched these user-visible contracts; `SketchViewModelContractTest` now pins the source order. `SketchViewModelContractTest`, `SketchDefaultsTest`, `SketchEraserSelectionTest`, and `SketchRepositoryTest` passed for this checkpoint.
