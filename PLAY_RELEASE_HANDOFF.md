...
- Current local HEAD before this code/documentation update: `60b9ea5 Align launch screen branding parity`.
- Working tree at save time: Android notification small icon branding code, regression tests, and documentation updates pending.
...
- Latest completed local parity checkpoint: Android notification small icon branding audit, committed in the current HEAD containing this file.
- Code-work note: Android local scheduled notifications and FCM system notifications now use the existing monochrome app drawable for `NotificationCompat.setSmallIcon()`, instead of the full-color launcher mipmap. This keeps Android status bar/notification shade rendering compatible with platform small-icon masking while preserving the app brand. `NotificationReceiverTest`, `FcmServiceTest`, and `AndroidManifestContractTest` passed for this checkpoint.
...
- Code-work resume point after this save: Android notification small icon branding behavior has been hardened and pinned by tests. If parity work resumes, continue with another user-visible secondary screen or integration path that has not been recently rechecked. If the user resumes the app release process, follow the Play release steps below and do not rebuild only because of this committed code change unless a new AAB is intentionally needed.
