...
- Working tree before this save: Shape Edit unsaved-cancel alert contract test and release-resume documentation updates pending.
...
- Latest completed local parity checkpoint: Shape Edit unsaved-cancel alert contract audit, committed in the current HEAD containing this file.
- Code-work note: iOS `ShapeEditView` shows an unsaved-changes alert only when `hasChanges()` is true, keeps cancel as a non-destructive dismiss, and uses destructive Close to discard and dismiss the sheet. Android already followed that behavior, and `ShapeEditContractTest` now pins the cancel handler, alert copy, destructive close action, and cancel-only dismissal. `ShapeEditContractTest`, `ShapeEditDefaultsTest`, and `StringResourceCoverageTest` passed for this checkpoint.
...
- Working tree status before this documentation save: Shape Edit unsaved-cancel alert contract test and release-resume documentation updates pending; after the save commit, rerun `git status --short` and expect a clean tree unless the user has made newer edits.
- Play internal test version already published: `3.5.5 (102) internal-1`.
- Do not rebuild or re-upload version code `102` just to resume the release process.
