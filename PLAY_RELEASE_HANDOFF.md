...
Saved on 2026-07-04 16:54 KST so a later session in this directory can continue the Android app release process when the user says "앱 출시 과정 다시 이어나가자".

- Resume trigger phrase from this directory: `앱 출시 과정 다시 이어나가자`.
- Current working branch at save time: `fix/critical-pri0-fixes`.
- Current local HEAD before this code/documentation update: `48a4ed7 Align floating tab bar bottom margin with iOS`.
- Working tree at save time: map shape-focus source-order contract test and documentation updates pending.
...
- Latest completed local parity checkpoint: map shape-focus and overlay-tap route audit, committed in the current HEAD containing this file.
- Code-work note: iOS `SavedTableListView` sends `MoveToShapeData(baseCoordinate, radius ?? 100, shapeID)`, and `MapViewModel` zooms first, waits 0.3s, updates highlight, then moves to the offset center. Android already matched this for saved-list focus and all shape types. A new source-order test now pins the iOS overlay-tap route: update selection/highlight first, keep the detail sheet closed, then emit the saved-list focus event so the map does not do an extra camera move when a visible overlay is tapped. `MapCameraFocusTest` and `SavedListSectionsTest` passed for this checkpoint.
...
Saved again on 2026-07-04 16:54 KST from this repository so a later session can resume the app release process when the user says "앱 출시 과정 다시 이어나가자". The Play Console internal-test release flow had already reached the published version detail page, and local Android/iOS parity audits later continued through the 2026-07-04 map shape-focus and overlay-tap route audit. The latest local code/parity checkpoint before this documentation save has not been uploaded as a newer Play build.

- Current working branch: `fix/critical-pri0-fixes`.
- Current local HEAD before this documentation save: `48a4ed7 Align floating tab bar bottom margin with iOS`.
- Working tree status before this documentation save: map shape-focus source-order contract test and documentation updates pending.
- Play internal test version already published: `3.5.5 (102) internal-1`.
- Do not rebuild or re-upload version code `102` just to resume the release process.
