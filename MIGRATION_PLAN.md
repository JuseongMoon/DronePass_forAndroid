...
- 도형 타입별 네이버 Maps 오버레이:
  - Circle → `NMFCircleOverlay`
  - Rectangle → `NMFPolygonOverlay` (4점)
  - Polygon → `NMFPolygonOverlay`
  - Polyline → `NMFPolylineOverlay`
...
**작업 내용**:
- SavedShapeListScreen (LazyColumn)
- 검색 기능 (SearchBar)
- 정렬 (최신순, 이름순, 날짜순)
- SavedShapeBottomSheet (도형 선택 시 하단 시트)
- 날짜/시간 선택기
- 도형 셀 UI (SavedShapeListItem)
- 탭 간 연동 (탭 전환 시 해당 도형으로 카메라 이동)
...
**작업 내용**:
- SketchViewModel (스케치 모드 상태 관리)
- 터치 이벤트 처리 (Compose `pointerInput` 또는 MapView 터치)
- 좌표 샘플링 (최소 5m 이상)
- Catmull-Rom 스플라인 보간 (SketchSmoothingAlgorithm)
- LRU 캐시 (SketchPointsCache)
- Undo/Redo 스택
- 지우개 모드 (BoundingBox 기반)
- 스케치 오버레이 (NMFPolylineOverlay)
- SketchToolbar (색상, 두께, 투명도)
...
**기술 리스크**: 네이버 MapView(View 기반)와 Compose의 터치 이벤트 충돌 가능. 스케치 모드 시 지도 터치 비활성화 필요.

---

### Phase 5: 설정 탭 + 드론 관리 (2주)

**목표**: 드론 CRUD, 앱 설정, 알림, 프로필

**작업 내용**:
- SettingsScreen (설정 메인)
- DroneListScreen / DroneEditScreen / DroneDetailScreen
- DroneManager (선택 드론, 색상 매핑)
- ColorManager (PaletteColor enum)
- ProfileScreen (로그인/로그아웃/회원탈퇴)
- 로컬 알림 (WorkManager + NotificationManager)
- 설정 영속화 (DataStore)
- 다국어 (strings.xml 한/영)
- AppInfoScreen (패치노트, 약관)
...
### Phase 6: VWorld 비행구역 + Kp 지수 (1.5주)

**목표**: 드론 비행구역 표시, 지자기 폭풍 지수

**작업 내용**:
- VWorldApiService (Retrofit, WFS GeoJSON 파싱)
- FlightZoneOverlayManager (레이어별 오버레이, 8개 레이어)
- FlightZoneLayerSelector UI
- VWorldZoneDetailSheet
- LRU 캐시 (15분 만료, 50개 제한)
- Debounce (Flow.debounce 500ms)
- FlightZoneCalculator (교차 판정, Haversine)
- KPIndexManager (GFZ Potsdam + NOAA API)
- KPForecastScreen
...
**작업 내용**:
- Open-Meteo API 연동 (WeatherKit 대체, 무료)
  - 풍속, 돌풍, 강수량, 기압, 습도
  - 시간별/일별 예보
- WeatherManager 포팅 (돌풍 평가 알고리즘 유지)
...
| **M2** | Circle/Rectangle/Polygon/Polyline 생성 + 지도 오버레이 |
