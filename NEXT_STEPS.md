...
- 2026-06-17 선택된 드론 칩 줄바꿈을 iOS `MainFloatingButtonView.FlowLayout(firstLineReservedWidth: 40)` 기준과 맞췄다. Android는 기존 `FlowRow`가 오른쪽 드롭다운 원 버튼 공간을 모든 줄에서 사실상 예약해 두 번째 줄 이후 칩이 iOS보다 빨리 감길 수 있었고, 이제 첫 줄만 40dp를 비우고 이후 줄은 전체 폭을 사용하도록 전용 레이아웃 계산을 고정했다. 기존 요청 기준인 드론 선택 버튼 높이 32dp와 오른쪽 원 지름 32dp, 상단 정렬도 유지한다. `:app:testDebugUnitTest --tests "*DroneSelectionDropdownTest" --tests "*MapScreenLayersTest" --tests "*MainScreenStartDestinationTest"`, `:app:assembleDebug` 통과.
