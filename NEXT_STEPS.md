...
- 2026-06-16 초기 `MIGRATION_PLAN.md`의 LWW 충돌 해결 품질 게이트를 최신 HEAD에서 재검증했다. `SyncMergeTest`는 local/server only, newer local/server, same timestamp server tie, mixed union, local/server soft delete, `filterServerNewer`, empty server, metadata update 등 10개 이상 케이스를 고정한다. 함께 돌린 remote delete/soft delete 주변 테스트도 통과했지만, 문서의 `Soft Delete 필터링 Integration Test`는 실제 통합 테스트 증거가 아니므로 아직 체크하지 않는다. `:app:testDebugUnitTest --tests "*SyncMergeTest" --tests "*ShapeRepositoryTest" --tests "*SketchRepositoryTest" --tests "*DroneSyncMergeTest" --tests "*FirestoreDeleteErrorTest"` 통과.
