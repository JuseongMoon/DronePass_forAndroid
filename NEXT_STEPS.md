...
- 2026-06-16 Soft Delete 필터링 통합 게이트를 Android 실기기 Room in-memory DB 테스트로 닫았다. `SoftDeleteFilteringIntegrationTest`는 Shape/Sketch/Drone DAO가 `deletedAt IS NULL` row만 활성 목록에 노출하고, 전체 목록에는 삭제 tombstone을 유지하는지 실제 SQLite/Room 런타임에서 검증한다. `RFCW324TZ0Z`(Android 15)에서 `:app:connectedDebugAndroidTest -Pandroid.testInstrumentationRunnerArguments.class=com.ScienceFiction.DronePassAndroid.core.data.local.room.SoftDeleteFilteringIntegrationTest` 실행, 3 tests 통과.
